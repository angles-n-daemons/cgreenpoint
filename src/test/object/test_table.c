...
void testBasic() {
  ...
  if (!newKey) {
    printf("testTable testBasic: expected key to be new, but tableSet returned "
           "false\n");
    exit(1);
  }
...
void testOverwriteKey() {
  ...
  if (newKey) {
    printf("testTable testOverwriteKey: expected key to not be new, but "
           "tableSet returned true\n");
    exit(1);
  }
...
void testDelete() {
  ...
  if (found) {
    printf("testTable testDelete: expected tableGet to return false on deleted "
           "key but was found\n");
    exit(1);
  }
}
